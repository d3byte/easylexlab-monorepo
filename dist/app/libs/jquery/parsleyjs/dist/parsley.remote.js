...
!function (factory) {
  ...
  ParsleyAbstract.prototype = {
    ...
    trigger: function trigger(name, target) {
      target = target || this;
      var queue = this._listeners && this._listeners[name];
  ...
  */
    ...
    Validator.prototype = {
      ...
      */
      validate: function validate(objectOrString, AssertsOrConstraintOrGroup, group) {
        if ('string' !== typeof objectOrString && 'object' !== (typeof objectOrString === 'undefined' ? 'undefined' : _typeof(objectOrString))) throw new Error('You must validate an object or a string');
      ...
      bind: function bind(object, constraint) {
        if ('object' !== (typeof object === 'undefined' ? 'undefined' : _typeof(object))) throw new Error('Must bind a Constraint to an object');
      ...
      _validateObject: function _validateObject(object, constraint, group) {
        if ('object' !== (typeof constraint === 'undefined' ? 'undefined' : _typeof(constraint))) throw new Error('You must give a constraint to validate an object');
    ...
    */
    ...
    Constraint.prototype = {
      ...
      add: function add(node, object) {
        ...
        if ('object' === (typeof object === 'undefined' ? 'undefined' : _typeof(object)) && !_isArray(object)) {
    ...
    Assert.prototype = {
      ...
      */
      ...
      Unique: function Unique(object) {
        ...
        if ('object' === (typeof object === 'undefined' ? 'undefined' : _typeof(object))) this.key = object.key;
        this.validate = function (array) {
          var value,
              store = [];
          if (!_isArray(array)) throw new Violation(this, array, { value: Validator.errorCode.must_be_an_array });
          for (var i = 0; i < array.length; i++) {
            value = 'object' === _typeof(array[i]) ? array[i][this.key] : array[i];
  ...
  ParsleyValidator.prototype = {
    ...
    addValidator: function addValidator(name, fn, priority, requirementsTransformer) {
      if (this.validators[name]) ParsleyUtils.warn('Validator "' + name + '" is already defined.');else if (ParsleyDefaults.hasOwnProperty(name)) {
    ...
    removeValidator: function removeValidator(name) {
      if (!this.validators[name]) ParsleyUtils.warn('Validator "' + name + '" is not defined.');
      delete this.validators[name];
      return this;
    },
    _setValidator: function _setValidator(name, fn, priority, requirementsTransformer) {
  ...
  ParsleyUI.prototype = {
    ...
    _insertErrorWrapper: function _insertErrorWrapper(fieldInstance) {
      ...
      if ('string' === typeof fieldInstance.options.errorsContainer) {
        if ($(fieldInstance.options.errorsContainer).length) return $(fieldInstance.options.errorsContainer).append(fieldInstance._ui.$errorsWrapper);else ParsleyUtils.warn('The errors container `' + fieldInstance.options.errorsContainer + '` does not exist in DOM');
  ...
  ParsleyField.prototype = {
    ...
    */
    ...
    _handleWhitespace: function _handleWhitespace(value) {
      if (true === this.options.trimValue) ParsleyUtils.warnOnce('data-parsley-trim-value="true" is deprecated, please use data-parsley-whitespace="trim"');
      if ('squish' === this.options.whitespace) value = value.replace(/\s{2,}/g, ' ');
      if ('trim' === this.options.whitespace || 'squish' === this.options.whitespace || true === this.options.trimValue) value = value.replace(/^\s+|\s+$/g, '');
      return value;
    },
    // Internal only.
  ...
  ParsleyFactory.prototype = {
    init: function init(options) {
      ...
      if (this.$element.is('form') || ParsleyUtils.checkAttr(this.$element, this.options.namespace, 'validate') && !this.$element.is(this.options.inputs)) return this.bind('parsleyForm');
    ...
    bind: function bind(type, doNotStore) {
      ...
      switch (type) {
        case 'parsleyForm':
          ...
          break;
        case 'parsleyField':
          parsleyInstance = $.extend(new ParsleyField(this.$element, this.domOptions, this.options, this.parent), window.ParsleyExtend);
          break;
        case 'parsleyFieldMultiple':
          parsleyInstance = $.extend(new ParsleyField(this.$element, this.domOptions, this.options, this.parent), new ParsleyMultiple(), window.ParsleyExtend)._init();
          break;
        default:
          throw new Error(type + 'is not a supported Parsley type');
      }
      if (this.options.multiple) ParsleyUtils.setAttr(this.$element, this.options.namespace, 'multiple', this.options.multiple);
  ...
  window.ParsleyExtend = $.extend(true, window.ParsleyExtend, {
    ...
    onSubmitValidate: function onSubmitValidate(event) {
      ...
      return this._asyncValidateForm(undefined, event).done(function () {
        ...
        if (that._trigger('submit') && !that.submitEvent.isDefaultPrevented()) that.$element.trigger($.extend($.Event('submit'), { parsley: true }));
      });
    },
...
//# sourceMappingURL=parsley.remote.js.map